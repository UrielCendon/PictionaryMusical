...
namespace PictionaryMusicalServidor.Pruebas.Servicios.Amigos
{
    ...
    public class AmigosManejadorPrueba
    {
        ...
        public void Inicializar()
        {
            ...
            _proveedorCallbackMock.Setup(p => p.ObtenerCallbackActual())
        ...
        public void Prueba_Suscribir_FlujoExitoso()
        {
            ...
            _operacionAmistadServicioMock.Setup(operacion => operacion.ObtenerDatosUsuarioSuscripcion(NombreUsuarioPrueba))
                .Returns(datosSuscripcion);

            _amigosManejador.Suscribir(NombreUsuarioPrueba);

            _manejadorCallbackMock.Verify(manejador => manejador.Suscribir(
                NombreUsuarioNormalizado,
                _callbackMock.Object), Times.Once);

            _notificadorAmigosMock.Verify(notificador => notificador.NotificarSolicitudesPendientesAlSuscribir(
                NombreUsuarioNormalizado,
                IdUsuarioPrueba), Times.Once);
        }
        ...
        public void Prueba_Suscribir_ErrorBaseDatos_LanzaFaultException()
        {
            ...
            Assert.ThrowsException<FaultException>(() => _amigosManejador.Suscribir(NombreUsuarioPrueba));
        }
        ...
        public void Prueba_EnviarSolicitudAmistad_FlujoExitoso()
        {
            ...
            _operacionAmistadServicioMock.Setup(operacion => operacion.EjecutarCreacionSolicitud(
                NombreUsuarioPrueba,
                NombreAmigoPrueba)).Returns(resultadoCreacion);

            _amigosManejador.EnviarSolicitudAmistad(NombreUsuarioPrueba, NombreAmigoPrueba);

            _notificadorAmigosMock.Verify(notificador => notificador.NotificarSolicitudActualizada(
                It.IsAny<string>(),
                It.IsAny<SolicitudAmistadDTO>()), Times.Once);
        }

        [TestMethod]
        public void Prueba_EnviarSolicitudAmistadUsuarioNoEncontrado_LanzaFaultException()
        {
            _operacionAmistadServicioMock.Setup(operacion => operacion.EjecutarCreacionSolicitud(
                NombreUsuarioPrueba,
                NombreAmigoPrueba)).Throws(new FaultException("Usuario no encontrado"));
        ...
        public void Prueba_ResponderSolicitudAmistad_FlujoExitoso()
        {
            ...
            _operacionAmistadServicioMock.Setup(operacion => operacion.EjecutarAceptacionSolicitud(
                NombreUsuarioPrueba,
                NombreAmigoPrueba)).Returns(resultadoAceptacion);
            ...
            _notificadorListaAmigosMock.Verify(notificador => notificador.NotificarCambioAmistad(
                NombreUsuarioNormalizado), Times.Once);
        }
        ...
        public void Prueba_EliminarAmigo_FlujoExitoso()
        {
            ...
            _operacionAmistadServicioMock.Setup(operacion => operacion.EjecutarEliminacion(
                NombreUsuarioPrueba,
                NombreAmigoPrueba)).Returns(resultadoEliminacion);

            _amigosManejador.EliminarAmigo(NombreUsuarioPrueba, NombreAmigoPrueba);

            _notificadorAmigosMock.Verify(notificador => notificador.NotificarAmistadEliminada(
                NombreUsuarioNormalizado,
                It.IsAny<SolicitudAmistadDTO>()), Times.Once);
        ...
        public void Prueba_CancelarSuscripcion_FlujoExitoso()
        {
            ...
            _manejadorCallbackMock.Verify(manejador => manejador.Desuscribir(NombreUsuarioPrueba), Times.Once);
        }

        [TestMethod]
        public void Prueba_CancelarSuscripcionNombreVacio_LanzaFaultException()
        {
            Assert.ThrowsException<FaultException>(() =>
